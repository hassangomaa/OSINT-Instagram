...
def cmdlist():
    ...
    print("Get target followers #with a limit up to your needs  ")
...
#create instance of OSINT instagram Library
api = Osintgram(args.id, args.file, args.json, args.command, args.output, args.cookies)



commands = {
    'list':             cmdlist,##if list write them all again
    'help':             cmdlist,
    'quit':             _quit,
    'exit':             _quit,
    'addrs':            api.get_addrs,
...
while True:
    if args.command:
        cmd = args.command# as long as the cmd exist in our list
        _cmd = commands.get(args.command)
    else:##if input not valid try agian
        signal.signal(signal.SIGINT, signal_handler)
        if is_windows:
            pyreadline.Readline().parse_and_bind("tab: complete")
            pyreadline.Readline().set_completer(completer)
        ...
        cmd = input()# read again 

        _cmd = commands.get(cmd)#store for the nxt check
